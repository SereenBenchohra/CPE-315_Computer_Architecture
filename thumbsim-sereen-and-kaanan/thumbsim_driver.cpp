...
bool Cache::access(unsigned int address) {
   unsigned int rows = size / blocksize;
   double bs = log2(blocksize);
   double index = log2(rows);
   unsigned int tagSize = 32 - (unsigned int)(bs+index);
   unsigned int tag = address >> (unsigned int) (bs + index);
   unsigned int indexBS = (address << tagSize);
   unsigned int justIndex = indexBS >>(tagSize + (unsigned int)bs);
   if(rows == 1){
      justIndex = 0;
   }
   if(entries[justIndex] == tag){
      hits++;
      return true;
   }
   entries[justIndex] = tag;
   misses++;
  return false;
}
